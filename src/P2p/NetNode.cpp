...
#undef INVOKE_HANDLER
  ...
  bool NodeServer::run() {
    ...
    logger(INFO) << "Stopping node_server and it's " << m_connections.size() << " connections...";
	interrupt_object(m_workingContextGroup);//z
    m_workingContextGroup.wait();

    logger(INFO) << "node_server loop stopped";
    return true;
  }
    ...
    try {
		...
		try {
			...
			System::Context<> timeoutContext(m_dispatcher, [&] {
				...
				interrupt_object(connectionContext);//z
		...
		try {
			...
			System::Context<> timeoutContext(m_dispatcher, [&] {	
				...
				interrupt_object(handshakeContext);//z
  ...
  bool NodeServer::try_ping(basic_node_data& node_data, P2pConnectionContext& context) {
    ...
    try {
      ...
      System::Context<> timeoutContext(m_dispatcher, [&] {
		...
		interrupt_object(pingContext);//z
      });
...
void NodeServer::on_connection_close(P2pConnectionContext& context)
  ...
  void NodeServer::connectionHandler(const boost::uuids::uuid& connectionId, P2pConnectionContext& ctx) {
    ...
    System::Context<> context(m_dispatcher, [this, &connectionId, &ctx] {
      ...
      interrupt_object(ctx);//z
	  interrupt_object(writeContext);//z
      writeContext.get();
...
void NodeServer::writeHandler(P2pConnectionContext& ctx) {
	...
	try {
		...
		interrupt_object(ctx); // stop connection on write error
	}
...
// --------------------------------
    template<typename T> void NodeServer::interrupt_object(T &obj){
        try
        {
            obj.interrupt();
        }
