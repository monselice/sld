...
//zz
			if (keys_seen.find(boost::get<KeyOutput>(out.target).key) != keys_seen.end()) {
				...
				if (error) {*error = "The same output target is present more than once";}				
				return false;
			}
...
//zz
			if (keys_seen.find(key) != keys_seen.end()) {
				...
				if (error) {*error = "The same multisignature output target is present more than once";}
				return false;
			}
  ...
  for (const auto &in : tx.inputs) {
    ...
    if (in.type() == typeid(KeyInput)) {
      ...
      if (boost::get<MultisignatureInput>(in).term != 0) {
        uint64_t hi;
        uint64_t lo = mul128(amount, 1, &hi);//$$ - deposit shit was here
...
//zz
#ifdef QCC_CODEBASE
	nebbiolo_slow_hash(bd.data(), bd.size(), res);
#else
