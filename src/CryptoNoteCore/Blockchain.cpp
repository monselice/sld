...
namespace std
{
  ...
  bool operator< (const Crypto::KeyImage & keyImage1,
		  const Crypto::KeyImage & keyImage2)
  {
...
namespace CryptoNote
{
  ...
  template < typename K, typename V, typename Hash >
    bool serialize (google::sparse_hash_map < K, V, Hash > &value,
  ...
  template < typename K, typename Hash >
    bool serialize (google::sparse_hash_set < K, Hash > &value,
		    Common::StringView name,
		    CryptoNote::ISerializer & serializer)
  {
...
// custom serialization to speedup cache loading
  bool serialize (std::vector < std::pair < Blockchain::TransactionIndex,
		  uint16_t >> &value, Common::StringView name,
		  CryptoNote::ISerializer & s)
  {
    const size_t elementSize =
      sizeof (std::pair < Blockchain::TransactionIndex, uint16_t >);
  ...
  public:
    BlockCacheSerializer (Blockchain & bs, const Crypto::Hash lastBlockHash,
			  ILogger & logger):m_bs (bs),
      m_lastBlockHash (lastBlockHash), m_loaded (false), logger (logger,
      ...
      else
	{
      ...
      logger (INFO) << "Serialization time: " << std::chrono::duration_cast <
	std::chrono::milliseconds > (dur).count () << "ms";
  ...
  public:
    BlockchainIndicesSerializer (Blockchain & bs,
...
Blockchain::Blockchain (const Currency & currency, tx_memory_pool & tx_pool, ILogger & logger):
  logger (logger, "Blockchain"),
    ...
    m_is_in_checkpoint_zone (false), m_checkpoints (logger)
...
//m_upgradeDetectorV4(currency, m_blocks, BLOCK_MAJOR_VERSION_4, logger) 
  {
    ...
    Crypto::KeyImage nullImage =
      boost::value_initialized < decltype (nullImage) > ();
  ...
  bool Blockchain::checkTransactionInputs (const CryptoNote::Transaction & tx,
					   ...
					   BlockInfo & lastFailed)
  {
	...
	if (!checkTransactionInputs
	    (tx, maxUsedBlock.height, maxUsedBlock.id, &tail))
	  {
	    lastFailed = tail;
	    return false;
	  }
...
// pre m_blockchain_lock is locked
			 ...
			 bool load_existing)
  {
	...
	&& !Tools::create_directories_if_necessary (config_folder))
      {
	logger (ERROR,
		BRIGHT_RED) << "Failed to create data directory: " <<
	  m_config_folder;
	return false;
      }
	...
	(appendPath (config_folder, m_currency.blocksFileName ()),
	 appendPath (config_folder, m_currency.blockIndexesFileName ()),
	 1024))
      {
	return false;
      }
	...
	BlockCacheSerializer loader (*this,
				     get_block_hash (m_blocks.back ().bl),
				     logger.getLogger ());
	loader.load (appendPath
		     (config_folder, m_currency.blocksCacheFileName ()));

	if (!loader.loaded ())
	  {
	    logger (WARNING,
		    BRIGHT_YELLOW) <<
	      "No blockchain cache found, rebuilding...";
	    rebuildCache ();
	  }
...
//zzx pushBlock(m_currency.genesisBlock(), get_block_hash(m_currency.genesisBlock()), bvc, 0);
	...
	if (!(firstBlockHash == m_currency.genesisBlockHash ()))
	  {
	    logger (ERROR,
		    BRIGHT_RED) << "Failed to init: genesis block mismatch. "
    ...
    logger (INFO, BRIGHT_GREEN)
      << "Blockchain initialized. last block: " << m_blocks.size () -
      1 << ", " << Common::timeIntervalToString (timestamp_diff) <<
...
*/
  ...
  void Blockchain::rebuildCache ()
  {
    ...
    for (uint32_t b = 0; b < m_blocks.size (); ++b)
      {
	if (b % REBUILD_CACHE_DISPLAY_STEP == 0)
	  {
	    logger (INFO,
		    BLUE) << "Height " << b << " of " << m_blocks.size ();
	  }
	...
	for (uint16_t t = 0; t < block.transactions.size (); ++t)
	  {
	    ...
	    m_transactionMap.insert (std::
				     make_pair (transactionHash,
						transactionIndex));
		...
		if (out.target.type () == typeid (KeyOutput))
		...
		else if (out.target.type () == typeid (MultisignatureOutput))
		  {
		    MultisignatureOutputUsage usage =
		      { transactionIndex, o, false };
		    m_multisignatureOutputs[out.amount].push_back (usage);
		  }
	      }

//zzx interest += m_currency.calculateTotalTransactionInterest(transaction.tx, b); //block.height);
	    interest +=
	      m_currency.calculateTotalTransactionInterest (transaction.tx);
	  }
    ...
    logger (INFO,
	    BRIGHT_WHITE) << "Rebuilding internal structures took: " <<
      duration.count ();
  }
  ...
  bool Blockchain::storeCache ()
  {
    ...
    if (!ser.save (appendPath
		   (m_config_folder, m_currency.blocksCacheFileName ())))
      {
  ...
  std::vector < Crypto::Hash >
    Blockchain::buildSparseChain (const Crypto::Hash & startBlockId)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
  ...
  std::vector < Crypto::Hash >
    Blockchain::doBuildSparseChain (const Crypto::Hash & startBlockId) const
  {
    assert (m_blockIndex.size () != 0);
	...
	std::copy (sparseMainChain.begin (), sparseMainChain.end (),
		   std::back_inserter (sparseChain));
      }
...
// --------------------------------
  bool Blockchain::getBlockHeight (const Crypto::Hash & blockId,
...
//zzx   uint8_t BlockMajorVersion = getBlockMajorVersionForHeight(static_cast<uint32_t>(m_blocks.size()));
//zzx   size_t offset = m_blocks.size() - std::min(m_blocks.size(), static_cast<uint64_t>(m_currency.difficultyBlocksCountByBlockVersion(BlockMajorVersion)));
    size_t offset = m_blocks.size () - std::min (m_blocks.size (),
	...
	commulative_difficulties.
	  push_back (m_blocks[offset].cumulative_difficulty);
      }
...
uint8_t Blockchain::for_height_get_block_major_version(uint64_t height) const {
...
#ifdef LEGACY_CODEBASE
    //SPAGHETTI code...
    if (height > Z_FORK_BLOCK_3)
      {
	return BLOCK_MAJOR_VERSION_4;
      }
    else if (height > Z_FORK_BLOCK_2)
      {
	return BLOCK_MAJOR_VERSION_3;
      }
    else if (height > Z_FORK_BLOCK_1)
      {
...
////////////////////////////////////////////////////////////////////////////
  bool Blockchain::rollback_blockchain_switching (std::list < Block >
						  &original_chain,
...
//zzx bool r = pushBlock(bl, get_block_hash(bl), bvc, ++height);
  ...
  bool Blockchain::switch_to_alternative_blockchain (std::list <
						     blocks_ext_by_hash::iterator
						     > &alt_chain,
						     bool
						     discard_disconnected_chain)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
...
//zzx
	...
	std::copy (b.transactionHashes.begin (), b.transactionHashes.end (),
		   std::inserter (mainChainTxHashes,
				  mainChainTxHashes.end ()));
      }
    for (auto alt_ch_iter = alt_chain.begin ();
	 alt_ch_iter != alt_chain.end (); alt_ch_iter++)
      {
    ...
    for (auto main_ch_it = mainChainTxHashes.begin ();
	 main_ch_it != mainChainTxHashes.end (); main_ch_it++)
      {
	auto tx_hash = *main_ch_it;
	if (std::find (altChainTxHashes.begin (), altChainTxHashes.end (),
		       tx_hash) == altChainTxHashes.end ())
	  {
	    logger (ERROR,
		    BRIGHT_RED) <<
...
//zzx
	...
	block_verification_context bvc =
	  boost::value_initialized < block_verification_context > ();
//zzx bool r = pushBlock(ch_ent->second.bl, get_block_hash(ch_ent->second.bl), bvc, ++height);  
	...
	if (!r || !bvc.m_added_to_main_chain)
	  {
		...
		m_orthanBlocksIndex.remove ((*alt_ch_to_orph_iter)->
					    second.bl);
	...
	//pushing old chain as alternative chain
      for (auto & old_ch_ent:disconnected_chain)
	  {
	    block_verification_context bvc =
	      boost::value_initialized < block_verification_context > ();
	    bool r = handle_alternative_block (old_ch_ent,
					       get_block_hash (old_ch_ent),
					       bvc,
					       false);
	    if (!r)
	      {
    ...
    blocksFromCommonRoot.push_back (alt_chain.front ()->second.
  ...
  for (auto ch_ent:alt_chain)
    ...
    sendMessage (BlockchainMessage
		 (ChainSwitchMessage (std::move (blocksFromCommonRoot))));

    logger (INFO,
	    BRIGHT_CYAN) << "REORGANIZE SUCCESS on height: " << split_height
      << ", new blockchain size: " << m_blocks.size ();
    return true;
  }

  difficulty_type Blockchain::get_next_difficulty_for_alternative_chain (const
									 std::
									 list
									 <
									 blocks_ext_by_hash::
...
//zzx if (alt_chain.size() < m_currency.difficultyBlocksCountByBlockVersion(BlockMajorVersion)) {
    if (alt_chain.size () < m_currency.difficultyBlocksCount ())
      {
	std::lock_guard < decltype (m_blockchain_lock) >
	  lk (m_blockchain_lock);
	size_t main_chain_stop_offset =
	  alt_chain.size ()? alt_chain.front ()->second.height : bei.height;
//zzx    if (!((alt_chain.size() + timestamps.size()) <= m_currency.difficultyBlocksCountByBlockVersion(BlockMajorVersion))) {
	size_t main_chain_count =
	  m_currency.difficultyBlocksCount () -
	  std::min (m_currency.difficultyBlocksCount (), alt_chain.size ());
	main_chain_count =
	  std::min (main_chain_count, main_chain_stop_offset);
	size_t main_chain_start_offset =
	  main_chain_stop_offset - main_chain_count;
	...
	for (; main_chain_start_offset < main_chain_stop_offset;
	     ++main_chain_start_offset)
	  {
	    timestamps.push_back (m_blocks[main_chain_start_offset].
				  bl.timestamp);
	    commulative_difficulties.push_back (m_blocks
						[main_chain_start_offset].cumulative_difficulty);
	  }

//zzx    if (!((alt_chain.size() + timestamps.size()) <= m_currency.difficultyBlocksCountByBlockVersion(BlockMajorVersion))) {
	if (!
	    ((alt_chain.size () + timestamps.size ()) <=
	     m_currency.difficultyBlocksCount ()))
	  {
	    logger (ERROR,
		    BRIGHT_RED) << "Internal error, alt_chain.size()[" <<
	      alt_chain.
	      size () << "] + timestamps.size()[" << timestamps.size () <<
	      "] NOT <= m_currency.difficultyBlocksCount()[" << m_currency.
	      difficultyBlocksCount () << ']';
	    return false;
	  }
      for (auto it:alt_chain)
	  {
	    ...
	    commulative_difficulties.push_back (it->
						second.cumulative_difficulty);
	  }
      }
    else
      {
	timestamps.resize (std::min (alt_chain.size (),
				     m_currency.difficultyBlocksCount ()));
	commulative_difficulties.resize (std::min (alt_chain.size (),
						   m_currency.
						   difficultyBlocksCount ()));
	size_t count = 0;
//$$
	if (timestamps.size () < 1)
	  {
	    logger (ERROR,
		    BRIGHT_RED) << "Internal error, timestamps.size() < 1";
	    return false;
	  }
...
//$$
						  ...
						  uint32_t height)
  {
	...
	logger (ERROR,
		BRIGHT_RED) <<
	  "<< Blockchain.cpp << coinbase transaction in the block shouldn't have more than 1 signature. Signature count: "
	  << b.baseTransaction.signatures.size ();
	return false;
      }
    ...
    if (boost::get < BaseInput > (b.baseTransaction.inputs[0]).blockIndex !=
	height)
      {
	logger (INFO,
		BRIGHT_RED) <<
	  "The miner transaction in block has invalid height: " << boost::get
	  < BaseInput >
	  (b.baseTransaction.
	   inputs[0]).blockIndex << ", expected: " << height;
	return false;
      }
//zz 
    logger (TRACE, CYAN) << "height: " << height;

    if (height < parameters::INITIAL_BLOCKS_TO_SKIP_Z)
      z_lock = height + 1;
    else if (height <
	     parameters::RUSH_BLOCKS_Z + parameters::INITIAL_BLOCKS_TO_SKIP_Z)
	...
	logger (ERROR, BRIGHT_RED)
	  ...
	  << b.baseTransaction.unlockTime << ", expected " << z_lock;
...
// --------------------------------
  ...
  for (auto & o:b.baseTransaction.outputs)
    ...
    get_last_n_blocks_sizes (lastBlocksSizes,
			     m_currency.rewardBlocksWindow ());
    ...
    if (!m_currency.getBlockReward (blocksSizeMedian, cumulativeBlockSize,
				    ...
				    reward, emissionChange, last_BlockHash))
      {
	logger (INFO, BRIGHT_WHITE)
	  << "block size "
	  << cumulativeBlockSize
...
// --------------------------------
  bool Blockchain::getBackwardBlocksSize (size_t from_height,
  ...
  bool Blockchain::get_last_n_blocks_sizes (std::vector < size_t > &sz,
					    size_t count)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    if (!m_blocks.size ())
					       ...
					       &timestamps)
  {
    ...
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    size_t need_elements =
      m_currency.timestampCheckWindow () - timestamps.size ();
    if (!(start_top_height < m_blocks.size ()))
      {
	logger (ERROR,
		BRIGHT_RED) << "internal error: passed start_height = " <<
	  start_top_height << " not less then m_blocks.size()=" <<
	  m_blocks.size ();
	return false;
      }
    size_t stop_offset =
      start_top_height > need_elements ? start_top_height - need_elements : 0;
    do
      {
...
// --------------------------------
  bool Blockchain::handle_alternative_block (const Block & b,
					     ...
					     block_verification_context & bvc,
					     bool
					     sendNewAlternativeBlockMessage)
  {
	...
	logger (ERROR, BRIGHT_RED)
	  << "Block with id: "
	  << Common::podToHex (id)
	  << " (as alternative) have wrong miner transaction" << ENDL;
...
///////////////////////////////////1234
    if (!m_checkpoints.is_alternative_block_allowed
	(getCurrentBlockchainHeight (), block_height))
      {
	logger (TRACE)
	  << "Block with id: "
	  << id
...
///////////////////////////////////1234
    ...
    const bool mainPrev =
      m_blockIndex.getBlockHeight (b.previousBlockHash, mainPrevHeight);
	...
	while (alt_it != m_alternative_chains.end ())
	  {
	    ...
	    alt_it =
	      m_alternative_chains.find (alt_it->second.bl.previousBlockHash);
	  }
	...
	if (alt_chain.size ())
	  {
	    ...
	    get_block_hash (m_blocks
			    [alt_chain.front ()->second.height - 1].bl, h);
	    ...
	    complete_timestamps_vector (alt_chain.front ()->second.height - 1,
					timestamps);
	...
	bei.height =
	  ...
	  (alt_chain.size ()? it_prev->second.height + 1 : mainPrevHeight +
	   1);
	...
	difficulty_type current_diff =
	  get_next_difficulty_for_alternative_chain (alt_chain, bei);
	...
	if (!m_currency.checkProofOfWork (m_cn_context, bei.bl, current_diff,
					  proof_of_work))
	  {
	    logger (INFO, BRIGHT_RED)
	      << "Block with id: " << id
	      ...
	      proof_of_work << ENDL << " expected difficulty: " <<
	      current_diff;
	...
	auto i_res =
	  m_alternative_chains.insert (blocks_ext_by_hash::
				       value_type (id, bei));
	...
	if (is_a_checkpoint)
	...
	else if (m_blocks.back ().cumulative_difficulty < bei.cumulative_difficulty)	//check if total chain difficulty bigger then in the main chain
	  {
	    ...
	    logger (INFO, CYAN)
	      ...
	      << bei.cumulative_difficulty << ENDL;
	  ...
	  {
	    logger (INFO, BRIGHT_BLUE) << "ALTERNATIVE BLOCK ADDED" << ENDL;
	    logger (INFO, BLUE)
	      << "ID : Diff = PoW: "
	      << id
	      << " : " << current_diff << " = " << proof_of_work << ENDL;
		...
		<< " - "
		<< bei.height
		<< " = " << h_delta << " !!! AHTUNG BLJAD!!!" << ENDL;
	    else
	      logger (INFO, BRIGHT_BLUE)
		<< "Chain Top - Block Height = Delta "
		<< getCurrentBlockchainHeight ()
		<< " - " << bei.height << " = " << h_delta << ENDL;

	    if (sendNewAlternativeBlockMessage)
	      {
		sendMessage (BlockchainMessage
			     (NewAlternativeBlockMessage (id)));
	      }
	    return true;
	  }
      }
	...
	logger (INFO, CYAN)
	  << "Alternative block "
	  << id << " is rejected since lost the parent";
      }
    return true;
  }
// --------------------------------
			      ...
			      std::list < Transaction > &txs)
  {
    ...
    for (size_t i = start_offset;
	 i < start_offset + count && i < m_blocks.size (); i++)
      {
...
// --------------------------------
  bool Blockchain::
    handleGetObjects (NOTIFY_REQUEST_GET_OBJECTS::request & arg,
	...
	if (!(!missed_tx_id.size ()))
	  {
	    logger (ERROR,
		    BRIGHT_RED) <<
	      "Internal error: have missed missed_tx_id.size()=" <<
	      missed_tx_id.size () << ENDL << "for block id = " <<
	      get_block_hash (bl);
	    return false;
	  }			//WTF???
...
// --------------------------------
  ...
  bool Blockchain::add_out_to_get_random_outs (std::vector < std::pair <
					       TransactionIndex,
					       uint16_t >> &amount_outs,
					       COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount
					       & result_outs, uint64_t amount,
					       size_t i)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
	...
	logger (ERROR,
	  ...
	  amount_outs[i].
	  second << " more than transaction outputs = " << tx.outputs.
	  size () << ", for tx id = " << getObjectHash (tx);
	return false;
      }
    if (!
	(tx.outputs[amount_outs[i].second].target.type () ==
	 typeid (KeyOutput)))
      {
    ...
    oen.out_key =
      boost::get < KeyOutput > (tx.outputs[amount_outs[i].second].target).key;
    return true;
  }

  size_t Blockchain::find_end_of_allowed_index (const std::vector <
						std::pair < TransactionIndex,
						uint16_t >> &amount_outs)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
  ...
  bool Blockchain::getRandomOutsByAmount (const
					  COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::request
					  & req,
					  COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::response
					  & res)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
  ...
  for (uint64_t amount:req.amounts)
	...
	if (it == m_outputs.end ())
	  {
	    logger (ERROR, BRIGHT_RED) <<
	      "COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS: not outs for amount "
	      << amount <<
	...
	if (!(up_index_limit <= amount_outs.size ()))
	  {
	    logger (ERROR,
	      ...
	      << up_index_limit << ", with amount_outs.size = " <<
	      amount_outs.size ();
	    return false;
	  }
	...
	if (up_index_limit > 0)
	  {
	    ShuffleGenerator < size_t,
	      Crypto::random_engine < size_t >> generator (up_index_limit);
	    for (uint64_t j = 0;
		 j < up_index_limit
		 && result_outs.outs.size () < req.outs_count; ++j)
	      {
		add_out_to_get_random_outs (amount_outs, result_outs, amount,
					    generator ());
	      }
	  }
      }
  ...
  uint32_t Blockchain::findBlockchainSupplement (const std::vector <
						 Crypto::Hash > &qblock_ids)
  {
  ...
  uint64_t Blockchain::blockDifficulty (size_t i)
  {
    ...
    if (!(i < m_blocks.size ()))
      {
	logger (ERROR,
		BRIGHT_RED) << "wrong block index i = " << i <<
    ...
    return m_blocks[i].cumulative_difficulty - m_blocks[i -
  ...
  void Blockchain::print_blockchain (uint64_t start_index, uint64_t end_index)
  {
    ...
    for (size_t i = start_index; i != m_blocks.size () && i != end_index; i++)
      {
	ss << "height " << i << ", timestamp " << m_blocks[i].
	  bl.timestamp << ", cumul_dif " << m_blocks[i].cumulative_difficulty
	  << ", cumul_size " << m_blocks[i].block_cumulative_size <<
	  "\nid\t\t" << get_block_hash (m_blocks[i].bl) << "\ndifficulty\t\t"
	  << blockDifficulty (i) << ", nonce " << m_blocks[i].bl.
	  nonce << ", tx_count " << m_blocks[i].bl.transactionHashes.
	  size () << ENDL;
      }
  ...
  for (const outputs_container::value_type & v:m_outputs)
      {
	const std::vector < std::pair < TransactionIndex, uint16_t >> &vals =
	  v.second;
	if (!vals.empty ())
	  {
  ...
  std::vector < Crypto::Hash >
    Blockchain::findBlockchainSupplement (const std::vector < Crypto::Hash >
					  &remoteBlockIds, size_t maxCount,
					  ...
					  uint32_t & startBlockIndex)
  {
    ...
    return m_blockIndex.getBlockIds (startBlockIndex,
  ...
  bool Blockchain::
    getTransactionOutputGlobalIndexes (const Crypto::Hash & tx_id,
				       std::vector < uint32_t > &indexs)
  {
	...
	logger (ERROR,
		BRIGHT_RED) <<
	  "internal error: global indexes for transaction " << tx_id <<
	  " is empty";
	return false;
      }
  ...
  bool Blockchain::get_out_by_msig_gindex (uint64_t amount, uint64_t gindex,
					   MultisignatureOutput & out)
  {
    ...
    auto & targetOut =
      transactionByIndex (msigUsage.transactionIndex).tx.
      outputs[msigUsage.outputIndex].target;
					   ...
					   BlockInfo * tail)
  {
    ...
    if (!(max_used_block_height < m_blocks.size ()))
      {
	logger (ERROR,
		BRIGHT_RED) << "internal error: max used block index=" <<
	  max_used_block_height << " is not less then blockchain size = " <<
	  m_blocks.size ();
	return false;
      }
  ...
  for (const auto & in:tx.inputs)
	...
	if (in.type () == typeid (KeyInput))
	  {
	    if (have_tx_keyimg_as_spent
		(boost::get < KeyInput > (in).keyImage))
	      {
		return true;
	      }
	  }
					   ...
					   uint32_t * pmax_used_block_height)
  {
    Crypto::Hash tx_prefix_hash =
      getObjectHash (*static_cast < const TransactionPrefix * >(&tx));
    return checkTransactionInputs (tx, tx_prefix_hash,
...
//$$
	    if (!isInCheckpointZone (getCurrentBlockchainHeight ()))
	      {
		if (!check_tx_input
		    (in_to_key, tx_prefix_hash, tx.signatures[inputIndex],
		     pmax_used_block_height))
		  {
		    logger (INFO, BRIGHT_RED)
...
*/
	    if (!validateInput
		(::boost::get < MultisignatureInput > (txin), transactionHash,
  ...
  bool Blockchain::is_tx_spendtime_unlocked (uint64_t unlock_time)
  {
	...
	if (getCurrentBlockchainHeight () - 1 +
	    m_currency.lockedTxAllowedDeltaBlocks () >= unlock_time)
	  return true;
	else
	  return false;
      }
	...
	if (current_time + m_currency.lockedTxAllowedDeltaSeconds () >=
	    unlock_time)
	  return true;
	else
	  return false;
  ...
  bool Blockchain::check_tx_input (const KeyInput & txin,
				   ...
				   const std::vector < Crypto::Signature >
				   &sig, uint32_t * pmax_related_block_height)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    ...
    struct outputs_visitor
    {
      std::vector < const Crypto::PublicKey * >&m_results_collector;
        Blockchain & m_bch;
      LoggerRef logger;
        outputs_visitor (std::vector <
			 const Crypto::PublicKey * >&results_collector,
			 Blockchain & bch,
			 ILogger &
			 logger):m_results_collector (results_collector),
	m_bch (bch), logger (logger, "outputs_visitor")
      {
      }

      bool handle_output (const Transaction & tx,
	...
	m_results_collector.push_back (&boost::get < KeyOutput >
				       (out.target).key);
	return true;
      }
    };
	...
	logger (INFO, BRIGHT_WHITE) <<
	  ...
	  m_currency.formatAmount (txin.
				   amount) << " and count indexes " <<
	...
	logger (INFO, BRIGHT_WHITE) <<
	  ...
	  outputIndexes.size () << " returned wrong keys count " <<
	  output_keys.size ();
	return false;
      }
    ...
    if (!(sig.size () == output_keys.size ()))
      {
	logger (ERROR,
		BRIGHT_RED) << "internal error: tx signatures count=" <<
	  sig.size () << " mismatch with outputs keys count for inputs=" <<
	  output_keys.size ();
	return false;
      }

//zzx   if (isInCheckpointZone(getCurrentBlockchainHeight())) {
	...
	if (out.target.type () == typeid (MultisignatureOutput))
	  {
	    if (tx.version < TRANSACTION_VERSION_2)
	      {
		logger (INFO,
			BRIGHT_WHITE) << getObjectHash (tx) <<
		  " contains multisignature output but have verion " <<
		  tx.version;
		return false;
	      }
	    else
	      {
		const auto & multisignatureOutput =::boost::get <
		  MultisignatureOutput > (out.target);
		if (multisignatureOutput.term != 0)
		  {
		    if (multisignatureOutput.term <
			m_currency.depositMinTerm ()
			|| multisignatureOutput.term >
			m_currency.depositMaxTerm ())
		      {
			logger (INFO,
				BRIGHT_WHITE) << getObjectHash (tx) <<
			  " multisignature output has invalid term: " <<
			  multisignatureOutput.term;
			return false;
		      }
			...
			logger (INFO,
				BRIGHT_WHITE) << getObjectHash (tx) <<
			  " multisignature output is a deposit output, but it has too small amount: "
			  << out.amount;
			return false;
		      }
		  }
...
//$$$$  
	...
	logger (INFO, BRIGHT_WHITE) <<
	  ...
	  b.timestamp << ", bigger than adjusted time +" << ts << " s.";
	return false;
      }
    ...
    size_t offset =
      m_blocks.size () <=
      m_currency.timestampCheckWindow ()? 0 : m_blocks.size () -
      m_currency.timestampCheckWindow ();
    for (; offset != m_blocks.size (); ++offset)
      {
	timestamps.push_back (m_blocks[offset].bl.timestamp);
      }
...
//////////////////////////////////////////////////////////
				      ...
				      const Crypto::Hash & blockHash)
  {
    ...
    const uint8_t expectedBlockVersion =
      for_height_get_block_major_version (height);
    if (b.majorVersion != expectedBlockVersion)
      {
	logger (INFO,
		BRIGHT_WHITE) << "Block " << blockHash <<
	  " has wrong major version: " << static_cast <
	  int >(b.majorVersion) << ", at height " << height <<
	  " expected version is " << static_cast <
  ...
  bool Blockchain::checkRootBlockSize (const Block & b,
				       const Crypto::Hash & blockHash)
  {
	...
	if (!getObjectBinarySize (serializer, rootBlockSize))
	  {
	    logger (ERROR, BRIGHT_RED) <<
	      "Block " << blockHash <<
	      ": failed to determine root block size";
	    return false;
	  }

	if (rootBlockSize > CryptoNote::MAX_ROOT_BLOCK_SIZE_Z)
	  {
	    logger (INFO, BRIGHT_WHITE)
	      << "Block "
	      << blockHash
	      << " contains too big root block: "
	      << rootBlockSize
	      << " bytes, expected no more than "
	      << CryptoNote::MAX_ROOT_BLOCK_SIZE_Z << " bytes";
...
// --------------------------------///
  bool Blockchain::checkCumulativeBlockSize (const Crypto::Hash & blockId,
					     ...
					     uint64_t height)
  {
    size_t maxBlockCumulativeSize =
      m_currency.maxBlockCumulativeSize (height);
    if (cumulativeBlockSize > maxBlockCumulativeSize)
      {
	logger (INFO, BRIGHT_WHITE) <<
	  "Block " << blockId << " is too big: " << cumulativeBlockSize <<
	  " bytes, " << "exptected no more than " << maxBlockCumulativeSize <<
	  " bytes";
	return false;
      }
...
// Returns true, if cumulativeSize is calculated precisely, else returns false.
  bool Blockchain::getBlockCumulativeSize (const Block & block,
					   size_t & cumulativeSize)
  {
...
// --------------------------------///
  bool Blockchain::addNewBlock (const Block & bl_,
				block_verification_context & bvc)
  {
...
//zzx add_result = pushBlock(bl, id, bvc, ++height);
    ...
    if (add_result && bvc.m_added_to_main_chain)
      {
	m_observerManager.notify (&IBlockchainStorageObserver::
				  blockchainUpdated);
      }
  ...
  const Blockchain::
    TransactionEntry & Blockchain::transactionByIndex (TransactionIndex index)
  {
...
*/
			      ...
			      block_verification_context & bvc)
  {
    ...
    if (blockData.previousBlockHash != getTailId ())
      {
	logger (INFO, BRIGHT_WHITE) <<
	  "Block " << blockHash << " has wrong previousBlockHash: " <<
    ...
    if (!check_block_timestamp_main (blockData))
      {
	logger (INFO, BRIGHT_WHITE) <<
	  "Block " << blockHash << " has invalid timestamp: " <<
	  blockData.timestamp;
	bvc.m_verification_failed = true;
	return false;
      }
    ...
    if (!(currentDifficulty))
      {
	logger (ERROR,
		BRIGHT_RED) << "!!!!!!!!! difficulty overhead !!!!!!!!!";
	return false;
      }
	...
	if (!m_checkpoints.check_block
	    (getCurrentBlockchainHeight (), blockHash))
	  {
	    logger (ERROR, BRIGHT_RED) << "CHECKPOINT VALIDATION FAILED";
	    bvc.m_verification_failed = true;
	    return false;
      ...
      {
	if (!m_currency.checkProofOfWork
	    (m_cn_context, blockData, currentDifficulty, proof_of_work))
	  {
	    logger (INFO, BRIGHT_WHITE) <<
	      "Block " << blockHash << ", has too weak proof of work: " <<
    ...
    if (!prevalidate_miner_transaction
	(blockData, static_cast < uint32_t > (m_blocks.size ())))
      {
	logger (INFO, BRIGHT_WHITE) <<
    ...
    Crypto::Hash minerTransactionHash =
      getObjectHash (blockData.baseTransaction);
    ...
    TransactionIndex transactionIndex =
      { block.height, static_cast < uint16_t > (0) };
    pushTransaction (block, minerTransactionHash, transactionIndex);

    size_t coinbase_blob_size =
      getObjectBinarySize (blockData.baseTransaction);
	...
	uint64_t fee =
	  m_currency.getTransactionAllInputsAmount (transactions[i]) -
	  getOutputAmount (transactions[i]);

	bool isTransactionValid = true;
	if (block.bl.majorVersion == BLOCK_MAJOR_VERSION_1
	    && transactions[i].version > TRANSACTION_VERSION_1)
	  {
	    isTransactionValid = false;
	    logger (INFO,
		    BRIGHT_WHITE) << "Block " << blockHash <<
	      " can't contain transaction " << tx_id <<
	      " because it has invalid version " << transactions[i].version;
	  }
	...
	if (!check_tx_outputs (transactions[i]))
	  {
	    isTransactionValid = false;
	    logger (INFO,
		    BRIGHT_WHITE) << "Transaction " << tx_id <<
	...
	if (!isTransactionValid)
	  {
	    logger (INFO,
		    BRIGHT_WHITE) << "Block " << blockHash <<
	      " has at least one invalid transaction: " << tx_id;
	    bvc.m_verification_failed = true;
...
//zzx    interestSummary += m_currency.calculateTotalTransactionInterest(transactions[i], block.height);
	interestSummary +=
	  m_currency.calculateTotalTransactionInterest (transactions[i]);
      }

//zzx  if (!checkCumulativeBlockSize(blockHash, cumulative_block_size, m_blocks.size())) {
    if (!checkCumulativeBlockSize
	(blockHash, cumulative_block_size, block.height))
      {
	...
	(blockData, block.height, cumulative_block_size,
	 already_generated_coins, fee_summary, reward, emissionChange,
	 getPrevBlockHash ()))
      {
	logger (INFO,
		BRIGHT_WHITE) << "Block " << blockHash <<
	  " has invalid miner transaction";
	bvc.m_verification_failed = true;
	popTransactions (block, minerTransactionHash);
	return false;
      }
    ...
    block.already_generated_coins =
      already_generated_coins + emissionChange + interestSummary;
    if (m_blocks.size () > 0)
      {
    ...
    logger (DEBUGGING) <<
      ...
      << ENDL << "HEIGHT " << block.height << ", difficulty:\t" <<
      currentDifficulty << ENDL << "block reward: " <<
      m_currency.formatAmount (reward) << ", fee = " <<
      m_currency.formatAmount (fee_summary) << ", coinbase_blob_size: " <<
      coinbase_blob_size << ", cumulative size: " << cumulative_block_size <<
...
//  m_upgradeDetectorV4.blockPushed();
  ...
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    return m_depositIndex.depositAmountAtHeight (static_cast <
						 DepositIndex::DepositHeight >
						 (height));
  }
/*    
...
*/
  ...
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    return m_depositIndex.depositInterestAtHeight (static_cast <
						   DepositIndex::DepositHeight
						   > (height));
  }
  ...
  void Blockchain::popBlock (const Crypto::Hash & blockHash)
  {
    ...
    std::vector < Transaction >
      transactions (m_blocks.back ().transactions.size () - 1);
    ...
    //zzx saveTransactions(transactions, height);
    saveTransactions (transactions);
...
//  m_upgradeDetectorV4.blockPopped();
  ...
  bool Blockchain::pushTransaction (BlockEntry & block,
				    const Crypto::Hash & transactionHash,
				    TransactionIndex transactionIndex)
  {
    auto result =
      m_transactionMap.insert (std::
			       make_pair (transactionHash, transactionIndex));
    if (!result.second)
      {
	logger (ERROR, BRIGHT_RED) <<
	...
	if (transaction.tx.inputs[i].type () == typeid (KeyInput))
	  {
	    auto result =
	      m_spent_keys.insert (::boost::get < KeyInput >
				   (transaction.tx.inputs[i]).keyImage);
	    if (!result.second)
	      {
		logger (ERROR, BRIGHT_RED) <<
		  "Double spending transaction was pushed to blockchain.";
		for (size_t j = 0; j < i; ++j)
		  {
		    m_spent_keys.erase (::boost::get < KeyInput >
					(transaction.tx.
					 inputs[i - 1 - j]).keyImage);
		  }
  ...
  for (const auto & inv:transaction.tx.inputs)
	...
	if (inv.type () == typeid (MultisignatureInput))
	  {
	    const MultisignatureInput & in =::boost::get <
	      MultisignatureInput > (inv);
	    auto & amountOutputs = m_multisignatureOutputs[in.amount];
	    amountOutputs[in.outputIndex].isUsed = true;
	  }
      }

    transaction.m_global_output_indexes.resize (transaction.tx.
						outputs.size ());
    for (uint16_t output = 0; output < transaction.tx.outputs.size ();
	 ++output)
      {
	if (transaction.tx.outputs[output].target.type () ==
	    typeid (KeyOutput))
	  {
	    auto & amountOutputs =
	      m_outputs[transaction.tx.outputs[output].amount];
	    transaction.m_global_output_indexes[output] =
	      static_cast < uint32_t > (amountOutputs.size ());
	    amountOutputs.push_back (std::
				     make_pair <> (transactionIndex, output));
	  }
	else if (transaction.tx.outputs[output].target.type () ==
		 typeid (MultisignatureOutput))
	  {
	    auto & amountOutputs =
	      m_multisignatureOutputs[transaction.tx.outputs[output].amount];
	    transaction.m_global_output_indexes[output] =
	      static_cast < uint32_t > (amountOutputs.size ());
	    MultisignatureOutputUsage outputUsage =
	      { transactionIndex, output, false };
				   ...
				   const Crypto::Hash & transactionHash)
  {
    ...
    for (size_t outputIndex = 0; outputIndex < transaction.outputs.size ();
	 ++outputIndex)
      {
	const TransactionOutput & output =
	  transaction.outputs[transaction.outputs.size () - 1 - outputIndex];
	if (output.target.type () == typeid (KeyOutput))
	  {
	    ...
	    if (amountOutputs->second.back ().first.block !=
		transactionIndex.block
		|| amountOutputs->second.back ().first.transaction !=
		transactionIndex.transaction)
	      {
		logger (ERROR, BRIGHT_RED) <<
	    ...
	    if (amountOutputs->second.back ().second !=
		transaction.outputs.size () - 1 - outputIndex)
	      {
		logger (ERROR, BRIGHT_RED) <<
	...
	else if (output.target.type () == typeid (MultisignatureOutput))
	  {
	    ...
	    if (amountOutputs->second.back ().transactionIndex.block !=
		transactionIndex.block
		|| amountOutputs->second.back ().
		transactionIndex.transaction != transactionIndex.transaction)
	      {
		logger (ERROR, BRIGHT_RED) <<
		  "Blockchain consistency broken - invalid transaction index.";
		continue;
	    ...
	    if (amountOutputs->second.back ().outputIndex !=
		transaction.outputs.size () - 1 - outputIndex)
	      {
		logger (ERROR, BRIGHT_RED) <<
		  "Blockchain consistency broken - invalid output index.";
  ...
  void Blockchain::popTransactions (const BlockEntry & block,
				    const Crypto::Hash & minerTransactionHash)
  {
    for (size_t i = 0; i < block.transactions.size () - 1; ++i)
      {
	popTransaction (block.transactions
			[block.transactions.size () - 1 - i].tx,
			block.bl.transactionHashes[block.
						   transactions.size () - 2 -
						   i]);
      }
				  ...
				  &transactionSignatures)
  {
    ...
    const MultisignatureOutputUsage & outputIndex =
      amountOutputs->second[input.outputIndex];
    ...
    const Transaction & outputTransaction =
      m_blocks[outputIndex.transactionIndex.block].
      transactions[outputIndex.transactionIndex.transaction].tx;
    ...
    assert (outputTransaction.outputs[outputIndex.outputIndex].amount ==
	    input.amount);
    assert (outputTransaction.outputs[outputIndex.outputIndex].
	    target.type () == typeid (MultisignatureOutput));
	...
	logger (DEBUGGING) << "Transaction << " << transactionHash <<
	  " contains multisignature input with invalid term.";
	return false;
      }

    if (output.term != 0
	&& outputIndex.transactionIndex.block + output.term >
	getCurrentBlockchainHeight ())
      {
	logger (DEBUGGING) << "Transaction << " << transactionHash <<
	  " contains multisignature input that spends locked deposit output";
	return false;
      }
	...
	if (Crypto::check_signature (transactionPrefixHash,
  ...
  bool Blockchain::removeLastBlock ()
  {
    ...
    logger (DEBUGGING) << "Removing last block with height " <<
      m_blocks.back ().height;
    popTransactions (m_blocks.back (),
		     getObjectHash (m_blocks.back ().bl.baseTransaction));
  ...
  bool Blockchain::getLowerBound (uint64_t timestamp, uint64_t startOffset,
				  uint32_t & height)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    ...
    auto bound =
      std::lower_bound (m_blocks.begin () + startOffset, m_blocks.end (),
			timestamp - m_currency.blockFutureTimeLimit (),
			[](const BlockEntry & b,
			   uint64_t timestamp) { return b.bl.timestamp <
			timestamp;}
    );
    ...
    height =
      static_cast < uint32_t > (std::distance (m_blocks.begin (), bound));
    return true;
  }

  std::vector < Crypto::Hash > Blockchain::getBlockIds (uint32_t startHeight,
							uint32_t maxCount)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
  ...
  bool Blockchain::getBlockContainingTransaction (const Crypto::Hash & txId,
						  Crypto::Hash & blockId,
						  uint32_t & blockHeight)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
  ...
  bool Blockchain::getAlreadyGeneratedCoins (const Crypto::Hash & hash,
  ...
  bool Blockchain::getMultisigOutputReference (const MultisignatureInput &
					       txInMultisig,
					       std::pair < Crypto::Hash,
					       size_t > &outputReference)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    MultisignatureOutputsContainer::const_iterator amountIter =
      m_multisignatureOutputs.find (txInMultisig.amount);
    if (amountIter == m_multisignatureOutputs.end ())
      {
	logger (DEBUGGING) <<
	  "Transaction contains multisignature input with invalid amount.";
	return false;
      }
    if (amountIter->second.size () <= txInMultisig.outputIndex)
      {
	logger (DEBUGGING) <<
	  "Transaction contains multisignature input with invalid outputIndex.";
	return false;
      }
    const MultisignatureOutputUsage & outputIndex =
      amountIter->second[txInMultisig.outputIndex];
    const Transaction & outputTransaction =
      m_blocks[outputIndex.transactionIndex.block].
      transactions[outputIndex.transactionIndex.transaction].tx;
  ...
  bool Blockchain::storeBlockchainIndices ()
  {
    ...
    BlockchainIndicesSerializer ser (*this, getTailId (),
				     logger.getLogger ());

    if (!storeToBinaryFile
	(ser,
	 appendPath (m_config_folder,
  ...
  bool Blockchain::loadBlockchainIndices ()
  {
    ...
    logger (INFO,
	    BRIGHT_WHITE) <<
      "Loading blockchain indices for BlockchainExplorer...";
    BlockchainIndicesSerializer loader (*this,
					get_block_hash (m_blocks.back ().bl),
					logger.getLogger ());

    loadFromBinaryFile (loader,
			appendPath (m_config_folder,
				    m_currency.blockchinIndicesFileName ()));
	...
	logger (WARNING,
		BRIGHT_YELLOW) <<
	  "No indices for BlockchainExplorer found, rebuilding...";
	std::chrono::steady_clock::time_point timePoint =
	  std::chrono::steady_clock::now ();
	...
	for (uint32_t b = 0; b < m_blocks.size (); ++b)
	  {
	    if (b % REBUILD_CACHE_DISPLAY_STEP == 0)
	      {
		logger (INFO,
			BLUE) << "# " << b << " of " << m_blocks.size ();
	      }
	    const BlockEntry & block = m_blocks[b];
	    m_timestampIndex.add (block.bl.timestamp,
				  get_block_hash (block.bl));
	    m_generatedTransactionsIndex.add (block.bl);
	    for (uint16_t t = 0; t < block.transactions.size (); ++t)
	      {
		const TransactionEntry & transaction = block.transactions[t];
	...
	std::chrono::duration < double >duration =
	  std::chrono::steady_clock::now () - timePoint;
	logger (INFO,
		BRIGHT_WHITE) << "Rebuilding blockchain indices took: " <<
	  duration.count ();
      }
    return true;
  }

  bool Blockchain::getGeneratedTransactionsNumber (uint32_t height,
						   uint64_t &
  ...
  bool Blockchain::getOrphanBlockIdsByHeight (uint32_t height,
					      std::vector < Crypto::Hash >
					      &blockHashes)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
  ...
  bool Blockchain::getBlockIdsByTimestamp (uint64_t timestampBegin,
  ...
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
    return m_timestampIndex.find (timestampBegin, timestampEnd,
				  blocksNumberLimit, hashes,
				  blocksNumberWithinTimestamps);
  }

  bool Blockchain::
    getTransactionIdsByPaymentId (const Crypto::Hash & paymentId,
				  std::vector < Crypto::Hash >
				  &transactionHashes)
  {
    std::lock_guard < decltype (m_blockchain_lock) > lk (m_blockchain_lock);
...
//zzx bool Blockchain::loadTransactions(const Block& block, std::vector<Transaction>& transactions, uint32_t height) {
  bool Blockchain::loadTransactions (const Block & block,
				     std::vector < Transaction >
				     &transactions)
  {
    ...
    for (size_t i = 0; i < block.transactionHashes.size (); ++i)
      {
	if (!m_tx_pool.take_tx (block.transactionHashes[i], transactions[i],
				transactionSize, fee))
	  {
...
//zzx   if (!m_tx_pool.add_tx(transactions[i - 1 - j], context, true, height)
		if (!m_tx_pool.add_tx
		    (transactions[i - 1 - j], context, true))
		  {
...
//zzx void Blockchain::saveTransactions(const std::vector<Transaction>& transactions, uint32_t height) {
  void Blockchain::saveTransactions (const std::vector < Transaction >
				     &transactions)
  {
    tx_verification_context context;
    for (size_t i = 0; i < transactions.size (); ++i)
      {
//zzx     if (!m_tx_pool.add_tx(transactions[transactions.size() - 1 - i], context, true, height)) {
	if (!m_tx_pool.add_tx
	    (transactions[transactions.size () - 1 - i], context, true))
	  {
	    throw std::runtime_error
  ...
  bool Blockchain::addMessageQueue (MessageQueue < BlockchainMessage >
				    &messageQueue)
  {
    return m_messageQueueList.insert (messageQueue);
  }

  bool Blockchain::removeMessageQueue (MessageQueue < BlockchainMessage >
				       &messageQueue)
  {
  ...
  {
    for (IntrusiveLinkedList < MessageQueue <
	 BlockchainMessage >>::iterator iter = m_messageQueueList.begin ();
