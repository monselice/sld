...
bool Blockchain::init(const std::string& config_folder, bool load_existing) {
  ...
  if (load_existing && !m_blocks.empty()) {
    ...
    if (!loader.loaded()) {
      logger(WARNING, BRIGHT_YELLOW) << "No blockchain cache found, rebuilding...";
      rebuildCache();
    }
...
logger(TRACE, CYAN) << "height: " << height;
	...
	if(height<parameters::INITIAL_BLOCKS_TO_SKIP_Z)
		z_lock = height + 1;
	else if (height < parameters::RUSH_BLOCKS_Z + parameters::INITIAL_BLOCKS_TO_SKIP_Z)
...
//zz
	if (last_BlockHash != NULL_HASH) { //don't check genesis block
...
bool Blockchain::checkBlockVersion(const Block& b, const Crypto::Hash& blockHash) {
  uint64_t height = get_block_height(b);
  const uint8_t expectedBlockVersion = for_height_get_block_major_version(height);
  ...
  if (b.majorVersion >= BLOCK_MAJOR_VERSION_3) {
    ...
    if (rootBlockSize > CryptoNote::MAX_ROOT_BLOCK_SIZE_Z) {
      logger(INFO, BRIGHT_WHITE) 
		...
		<< CryptoNote::MAX_ROOT_BLOCK_SIZE_Z
		<< " bytes";
...
bool Blockchain::loadBlockchainIndices() {
  ...
  if (!loader.loaded()) {
    logger(WARNING, BRIGHT_YELLOW) << "No indices for BlockchainExplorer found, rebuilding...";
    std::chrono::steady_clock::time_point timePoint = std::chrono::steady_clock::now();
