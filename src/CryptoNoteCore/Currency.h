...
//  uint32_t calculateUpgradeHeight(uint32_t voteCompleteHeight) const { return voteCompleteHeight + m_upgradeWindow; }
  ...
  bool checkProofOfWorkV7(Crypto::cn_context& context, const Block& block, difficulty_type currentDiffic, Crypto::Hash& proofOfWork) const;
...
private:
  ...
  bool generateGenesisBlock();
	...
	uint64_t CalcReward(uint64_t alreadyGeneratedCoins, uint32_t height, const Crypto::Hash last_BlockHash = NULL_HASH) const;
	uint64_t CalcLockDuration(uint32_t _height, const Crypto::Hash _last_block_hash) const;
	uint64_t Lottery(const Crypto::Hash lbh) const;
	uint64_t LotteryRewardFactor(const uint64_t _level) const;
	uint64_t LotteryLockFactor(const uint64_t _level) const;
...
public:
  ...
  Transaction generateGenesisTransaction();
	Transaction generateGenesisTxNew();
