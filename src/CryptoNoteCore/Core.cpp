...
bool core::get_block_template(Block& b, const AccountPublicAddress& adr, difficulty_type& diffic, uint32_t& height, const BinaryArray& ex_nonce) {
...

	{
		...
		b.majorVersion = m_blockchain.for_height_get_block_major_version(height);
	...
	bool r = m_currency.constructMinerTx(height, median_size, already_generated_coins, txs_size, fee, adr, b.baseTransaction, ex_nonce, parameters::X_MAX_OUTPUTS_COUNT_Z, m_blockchain.getPrevBlockHash());
	if (!r) { 
	...
	for (size_t try_count = 0; try_count != 10; ++try_count) {
		r = m_currency.constructMinerTx(height, median_size, already_generated_coins, cumulative_size, fee, adr, b.baseTransaction, ex_nonce, parameters::X_MAX_OUTPUTS_COUNT_Z, m_blockchain.getPrevBlockHash());
