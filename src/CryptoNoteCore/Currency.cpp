...
bool Currency::init() {
  ...
  if (!get_block_hash(m_genesisBlock, m_genesisBlockHash)) {
		logger(ERROR, BRIGHT_RED) 
			<< "Failed to get genesis block hash";
    return false;
  }
...
bool Currency::generateGenesisBlock() {
  ...
  BinaryArray minerTxBlob;
	...
	//SPAGHETTI code...
	bool r = fromHex(genesisCoinbaseTxHex, minerTxBlob) && fromBinaryArray(m_genesisBlock.baseTransaction, minerTxBlob);
#endif
#ifdef PASSATELLI
	//PASSATELLI code...
	bool r = fromHex(genesisCoinbaseTxHex, minerTxBlob) && fromBinaryArray(m_genesisBlock.rootBlock.baseTransaction, minerTxBlob);
#endif
...
#ifdef PASSATELLI
	//PASSATELLI code...
	m_genesisBlock.majorVersion = BLOCK_MAJOR_VERSION_7;
#endif
...
#ifdef PASSATELLI
//zz
	m_genesisBlock.rootBlock.majorVersion = BLOCK_MAJOR_VERSION_7;
	m_genesisBlock.rootBlock.minorVersion = BLOCK_MINOR_VERSION_0;
	m_genesisBlock.rootBlock.transactionCount = 1;
	TransactionExtraMergeMiningTag mm_tag = boost::value_initialized<decltype(mm_tag)>();

	if (!appendMergeMiningTagToExtra(m_genesisBlock.rootBlock.baseTransaction.extra, mm_tag)) {
		logger(ERROR, BRIGHT_RED) 
			<< "Failed to append merge mining tag to extra of the genesis root block";
...
////////////////////////////////////////////////////////////////////////////
uint64_t Currency::CalcLockDuration(uint32_t _height, const Crypto::Hash _last_block_hash) const {	
#ifdef LEGACY_CODEBASE
	//SPAGHETTI code...
	...
	if(_height<2)
		z_lock = _height + 1;
	else if (_height < parameters::RUSH_BLOCKS_Z + 2)
		z_lock = _height + parameters::RUSH_REWARD_LOCK_Z;
	else
		z_lock = _height + m_minedMoneyUnlockWindow;
	
	return z_lock;
#endif
#ifdef PASSATELLI
	//PASSATELLI code...
	uint64_t lottery_lock_factor = 0;
	
	lottery_lock_factor = LotteryLockFactor(Lottery(_last_block_hash));
	
	//ordinary blocks
	if (height>=parameters::INITIAL_BLOCKS_TO_SKIP_Z+parameters::RUSH_BLOCKS_Z) {return _height + (m_minedMoneyUnlockWindow * lottery_lock_factor);}

	//rush mining and premine
	if (height<parameters::INITIAL_BLOCKS_TO_SKIP_Z) {//zero or zero+first - premined blocks
		return 1;//not implemented yet
	} else {//rush mining blocks
			//_height +
			//m_minedMoneyUnlockWindow = 12 *
			//rush_factor = 50 *
			//k_factor = 40 *
			//lottery_lock_factor = 1..3 *
			//interval_factor = 1..4
...
////////////////////////////////////////////////////////////////////////////
uint64_t Currency::CalcReward(uint64_t alreadyGeneratedCoins, uint32_t height, const Crypto::Hash last_BlockHash) const {

	uint64_t prize_factor = 0;
	
	if (alreadyGeneratedCoins>parameters::MONEY_SUPPLY_MINEABLE_Z) {return 1;}//coins already mined, return ashe...	
	
	prize_factor = LotteryRewardFactor(Lottery(last_BlockHash));

#ifdef SOLDO_CODEBASE 
	//for Soldo only reduce reward
	if (height>=Z_FORK_BLOCK_7) {return parameters::COMMON_REWARD_Z*prize_factor/100;}
#endif
	
	//reserved for a future fork
	//if (height>=Z_FORK_BLOCK_10) {return 1212;}

	//ordinary blocks
	if (height>=parameters::RUSH_BLOCKS_Z+parameters::INITIAL_BLOCKS_TO_SKIP_Z){return parameters::COMMON_REWARD_Z*prize_factor;}

	//rush mining and premine
	if (height<parameters::INITIAL_BLOCKS_TO_SKIP_Z) {//zero or zero+first - premined blocks
		if (height==0) {return parameters::ZERO_BLOCK_REWARD_Z;}
		if (height==1) {return parameters::FIRST_BLOCK_REWARD_Z;}
	} else {//rush mining blocks
#ifdef LEGACY_CODEBASE
		//SPAGHETTI code...
		if (prize_factor>1)
			{return parameters::RUSH_REWARD_Z*prize_factor*5;}
		else
			{return parameters::RUSH_REWARD_Z;}
#endif
#ifdef PASSATELLI
		//PASSATELLI code...
		return 1;
...
uint64_t Currency::Lottery(const Crypto::Hash lbh) const {
	...
	if (lbh == NULL_HASH) { //genesis block - return 0
		return level;
	}	
	...
	logger(TRACE) << "LEVEL = " << to_string(level);
	return level;
}
////////////////////////////////////////////////////////////////////////////
uint64_t Currency::LotteryRewardFactor(const uint64_t _level) const {
	...
	logger(TRACE) << "LEVEL = " << to_string(_level) << " FACTOR = " << factor;
	return factor;
}
////////////////////////////////////////////////////////////////////////////
uint64_t Currency::LotteryLockFactor(const uint64_t _level) const {
...
bool Currency::getBlockReward(size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins,
  uint64_t fee, uint32_t height, uint64_t& reward, int64_t& emissionChange, const Crypto::Hash last_BlockHash) const {
	...
	uint64_t baseReward = CalcReward(alreadyGeneratedCoins, height, last_BlockHash);
...
//zz
	...
	tx.unlockTime = CalcLockDuration(height, last_BlockHash);
...
// --------------------------------///
Transaction CurrencyBuilder::generateGenesisTxNew() {

	CryptoNote::AccountPublicAddress address;
	const std::string address_string = CryptoNote::GENESIS_WALLET_Z;
    uint64_t target_amount = parameters::GENESIS_BLOCK_REWARD_Z;
	uint64_t prefix;
	...
	r = Crypto::derive_public_key(derivation, 0, address.spendPublicKey, outEphemeralPubKey);
	assert(r == true);
	...
	out.amount = target_amount;
//	std::cout << "out.amount = " << std::to_string(out.amount) << std::endl;
//	std::cout << "viewPublicKey: " << address.viewPublicKey << std::endl;
//	std::cout << "spendPublicKey: " << address.spendPublicKey << std::endl;
	out.target = tk;
	tx.outputs.push_back(out);
