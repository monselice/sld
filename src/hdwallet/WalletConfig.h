...
namespace WalletConfig
{
    ...
    const std::string addressPrefix = CRYPTONOTE_ASSET_WALLET_ADDRESS_PREFIX;

    /* Your coins 'Ticker', e.g. Monero = XMR, Bitcoin = BTC */
    const std::string ticker = CRYPTONOTE_ASSET_TICKER;

    /* The filename to output the CSV to in save_csv */
    const std::string csvFilename = "txs.csv";

    /* The filename to read+write the address book to - consider starting with
       a leading '.' to make it hidden under mac+linux */
    const std::string addressBookFilename = ".contacts.txt";

    /* The name of your deamon */
    const std::string daemonName = CRYPTONOTE_ASSET_DAEMON;

    /* The name to call this wallet */
    const std::string walletName = CRYPTONOTE_ASSET_WALLET_HD;

    /* The name of walletd, the programmatic rpc interface to a wallet */
    const std::string walletdName = CRYPTONOTE_ASSET_WALLET;

    /* The full name of your crypto */
    const std::string coinName = CRYPTONOTE_ASSET_NAME;

    /* Where can your users contact you for support? E.g. discord */
    const std::string contactLink = CRYPTONOTE_ASSET_LINK_COMMUNITY;
