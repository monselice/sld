...
#include <hdwallet/PasswordContainer.h>
#include <hdwallet/WalletConfig.h>

bool saveWallet(std::shared_ptr<WalletInfo> &walletInfo)
{
    std::fstream tempFile;
    std::string tempFilePath = createTemporaryFile(walletInfo->walletFileName,
                                                   tempFile);
    ...
    catch (const std::exception &)
    {
        ...
        std::cout << WarningMsg("Failed to save wallet! Ensure you can write ")
                  << WarningMsg("to the current directory.") << std::endl;
    ...
    Tools::replace_file(tempFilePath, walletInfo->walletFileName);
                ...
                std::string walletPass)
{
    ...
    if (!walletFile)
    {
        std::cout << WarningMsg("Failed to load wallet! Ensure you can read ")
                  << WarningMsg("from the current directory.") << std::endl;
...
{
    return loadWallet(walletInfo->wallet, walletInfo->walletFileName,
...
std::string formatAmount(uint64_t amount)
{
    ...
    const uint64_t divisor
		= static_cast<uint64_t>(pow(10, WalletConfig::numDecimalPlaces));
